# Medusa Frontend - Docker Контейнеризация

## 📋 Обзор

Этот проект подготовлен для работы в Docker контейнере с возможностью деплоя на VPS через GitHub.

## 🚀 Быстрый старт

### Вариант 1: Без Nginx (для разработки или без домена)

```bash
# Запуск в Docker
docker-compose up -d

# Просмотр логов
docker-compose logs -f

# Остановка
docker-compose down
```

### Вариант 2: С Nginx в Docker (для production с доменом)

```bash
# Запуск с Nginx (требуется SSL сертификаты)
docker-compose -f docker-compose-nginx.yml up -d

# Просмотр логов
docker-compose -f docker-compose-nginx.yml logs -f

# Остановка
docker-compose -f docker-compose-nginx.yml down
```

**Примечание**: Для production с доменом рекомендуется использовать Nginx на хосте (см. [`NGINX_SETUP.md`](NGINX_SETUP.md)).

### Деплой на VPS

```bash
# На VPS (без Nginx)
./deploy.sh

# На VPS (с Nginx в Docker)
docker-compose -f docker-compose-nginx.yml up -d
```

Подробная инструкция в [`DEPLOYMENT.md`](DEPLOYMENT.md).

## 📁 Структура файлов

```
medusa-storefront/
├── Dockerfile                    # Multi-stage Docker конфигурация
├── .dockerignore                 # Исключения для Docker
├── docker-compose.yml            # Docker Compose конфигурация
├── .env.production              # Production переменные окружения
├── deploy.sh                     # Скрипт автоматического деплоя
├── DEPLOYMENT.md                 # Инструкция по деплою на VPS
├── DOCKER.md                     # Документация по Docker
└── DOCKER_README.md              # Этот файл
```

## 🔧 Конфигурация

### Переменные окружения

Создайте `.env.production` на основе `.env.template`:

```env
# Backend URL
MEDUSA_BACKEND_URL=http://88.218.67.164:9000

# Frontend URL
NEXT_PUBLIC_BASE_URL=http://88.218.67.164:8000

# Publishable API ключ
NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY=ваш-ключ

# Регион по умолчанию
NEXT_PUBLIC_DEFAULT_REGION=eu

# Секрет для revalidation (измените на случайную строку!)
REVALIDATE_SECRET=ваш-случайный-секрет
```

**Важно**: Файл `.env.production` не должен коммититься в Git!

## 🐳 Docker Команды

### Управление контейнерами

```bash
# Запуск
docker-compose up -d

# Остановка
docker-compose down

# Перезапуск
docker-compose restart

# Статус
docker-compose ps
```

### Логи

```bash
# Все логи
docker-compose logs

# Логи в реальном времени
docker-compose logs -f

# Последние 50 строк
docker-compose logs --tail=50
```

### Сборка

```bash
# Сборка с кэшем
docker-compose build

# Сборка без кэша
docker-compose build --no-cache
```

## 📦 Multi-stage Build

Dockerfile использует multi-stage build для оптимизации:

1. **Dependencies** - Установка зависимостей
2. **Builder** - Сборка Next.js приложения
3. **Runner** - Production runtime

Это минимизирует размер финального образа и улучшает безопасность.

## 🔒 Безопасность

- Контейнер запускается от имени не-root пользователя
- Переменные окружения хранятся в `.env.production`
- Health check для проверки работоспособности
- Логи ограничены по размеру (10MB, 3 файла)

## 🌐 Сеть

Frontend подключается к сети `medusa_network` для связи с backend.

Убедитесь, что сеть создана:

```bash
docker network create medusa_network
```

## 📊 Мониторинг

```bash
# Статистика контейнера
docker stats medusa_frontend

# Использование диска
docker system df

# Health status
docker inspect medusa_frontend | grep -A 10 Health
```

## 🚨 Troubleshooting

### Контейнер не запускается

```bash
# Проверьте логи
docker-compose logs

# Проверьте наличие .env.production
ls -la .env.production

# Проверьте конфигурацию
docker-compose config
```

### Ошибки сборки

```bash
# Очистите кэш
docker system prune -a

# Пересоберите без кэша
docker-compose build --no-cache
```

### Проблемы с сетью

```bash
# Проверьте сеть
docker network inspect medusa_network

# Проверьте подключение к backend
docker-compose exec frontend ping medusa_backend
```

## 📚 Документация

- [`DEPLOYMENT.md`](DEPLOYMENT.md) - Подробная инструкция по деплою на VPS
- [`DOCKER.md`](DOCKER.md) - Полная документация по Docker
- [`README.md`](README.md) - Общая документация проекта

## 🔗 Полезные ссылки

- [Docker Documentation](https://docs.docker.com/)
- [Docker Compose Documentation](https://docs.docker.com/compose/)
- [Next.js Deployment](https://nextjs.org/docs/deployment)
- [Medusa Documentation](https://docs.medusajs.com/)

## 📝 Примечания

1. **CORS**: Backend должен разрешать запросы с frontend URL. Настройки обновлены в `medusa-store/.env`.
2. **Health Check**: Контейнер проверяет HTTP ответ на `http://localhost:8000`.
3. **Логи**: Логи хранятся в Docker и ограничены по размеру.
4. **Обновление**: При деплое старые контейнеры корректно останавливаются.

## 🎯 Следующие шаги

### Базовый деплой (без домена)

1. Создайте `.env.production` на VPS
2. Запустите `./deploy.sh` на VPS
3. Проверьте работоспособность по адресу `http://88.218.67.164:8000`

### Production деплой (с доменом и HTTPS)

1. **Настройка домена**
   - Купите домен (например, example.com)
   - Настройте DNS A запись на IP: `88.218.67.164`
   - Подождите распространения DNS (обычно 1-24 часа)

2. **Настройка Nginx и SSL**
   - Следуйте инструкции в [`NGINX_SETUP.md`](NGINX_SETUP.md)
   - Установите Nginx и Certbot
   - Получите SSL сертификат (Let's Encrypt)
   - Настройте reverse proxy

3. **Обновление переменных окружения**
   - Измените `NEXT_PUBLIC_BASE_URL` на `https://example.com`
   - Измените `MEDUSA_BACKEND_URL` на `https://example.com/api`
   - Обновите CORS настройки в backend

4. **Перезапуск контейнеров**
   - Запустите `./deploy.sh` на VPS

5. **Проверка**
   - Откройте `https://example.com` - Frontend
   - Откройте `https://example.com/admin` - Admin панель
   - Проверьте HTTPS сертификат

Подробная инструкция в [`NGINX_SETUP.md`](NGINX_SETUP.md).

## 💬 Поддержка

При возникновении проблем:

1. Проверьте логи: `docker-compose logs`
2. Проверьте статус: `docker-compose ps`
3. Ознакомьтесь с [`DEPLOYMENT.md`](DEPLOYMENT.md) - раздел Troubleshooting
4. Обратитесь к документации Medusa и Next.js
